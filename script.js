...
document.getElementById('assignBtnreception').addEventListener('click', () => {
  ...
  const receptionemployees = employees.filter(emp => emp.role === "receptionist" || emp.role === "manager" || emp.role === "nettoyage" || emp.role === "autres");
...
document.getElementById('assignBtnconferenceroom').addEventListener('click', () => {
  let assignemployees = document.getElementById("cardlistassign");
  const conferenceroomemployees = employees.filter(emp => emp.role === "manager");
...
document.getElementById('assignBtnsecurity').addEventListener('click', () => {
  let assignemployees = document.getElementById("cardlistassign");
  const securityemployees = employees.filter(emp => emp.role === "security" || emp.role === "manager" || emp.role === "nettoyage")
  assignemployees.innerHTML = renderEmployees(securityemployees);
});

document.getElementById('assignBtnserversroom').addEventListener('click', () => {
  let assignemployees = document.getElementById("cardlistassign");
  const serversemployees = employees.filter(emp => emp.role === "technicien" || emp.role === "manager" || emp.role === "nettoyage");
  assignemployees.innerHTML = renderEmployees(serversemployees);
});

document.getElementById('assignBtnvault').addEventListener('click', () => {
  let assignemployees = document.getElementById("cardlistassign");
  const vaultemployees = employees.filter(emp => emp.role === "manager" || emp.role === "security" );
...
document.getElementById('assignBtnstaffroom').addEventListener('click', () => {
  ...
  profilmodal.querySelector(".modal-title").innerText = emp ? emp.name : 'Profil employé';
